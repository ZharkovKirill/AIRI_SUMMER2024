...
for tire in tires:
    ...
    tire.add_sensor("pos", "jointpos")

    tire.add_sensor("vel","jointvel")
...
for link in links:
    ...
    link.add_sensor("pos", "jointpos")
    link.add_sensor("vel", "jointvel")
...
env = sim.Environment("models/robot.xml", "models/oneslide_scene25deg250h.xml")
...
def controller(model, data):
    ...
    X_des = np.array([p_des, 0.0, 0.14, v_des, 0, 0])
    u_0 = np.array([0,0])
    u = u_0 - K @ (X - X_des)
    ...
    #     left_wheel -= 7 * np.abs(del_pos_l) if np.abs(del_pos_l) <= 0.8 else 0.8 * 6
        left_wheel -= k_alpha * X[1]
    elif np.round(gamma,2) < 0:
        del_pos_r = -err_gamma * k_gamma
    #     right_wheel += 7 * np.abs(del_pos_r) if np.abs(del_pos_r) <= 0.8 else 0.8 * 6
