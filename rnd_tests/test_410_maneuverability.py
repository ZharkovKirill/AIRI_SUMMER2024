...
def controller(model, data):
    ...
    if data.time > 0.1 and alpha < 0:
        arr_alpha_angle.append(np.deg2rad(360)+ alpha)
    elif data.time <= 0.8:
        arr_alpha_angle.append(alpha) 
    else:
        arr_alpha_angle.append(np.deg2rad(360)+alpha)
    ...
    if data.time >= 0.98:
        des_dalpha = 0
        alpha_des = 0
    X_des = np.array([p_des, alpha_des, beta_angle_correction, 0, des_dalpha, 0])
    u_0 = np.array([0,0])
    u = u_0 - K @ (X - X_des)
...
arr_alpha_angle = np.array(arr_alpha_angle)
...
ax[-1].plot(time_arr, np.rad2deg(arr_alpha_angle), "-",linewidth = 1.5, label="p")
ax[-1].set_xlabel("Время, [c]")
ax[-1].set_ylabel("Поворот робота, [град]")
ax[-1].grid()
# ax[-1].legend()
...
plt.savefig("footages/test_410_maneuverability.svg")
# %%
