...
def controller(model, data):
    ...
    states.append(X - np.array([0, 0, beta_angle_correction, 0, 0, 0]))
...
# # 
ax.append(plt.subplot(3, 1, 3))
ax[-1].plot(time_arr, actual_com_robot, "-",linewidth = 1.5, label="actual")
ax[-1].set_xlabel("Время, [c]")
ax[-1].set_ylabel("Высота корпуса, [м]")
ax[-1].grid()
plt.savefig("footages/test_44_stab.svg")
