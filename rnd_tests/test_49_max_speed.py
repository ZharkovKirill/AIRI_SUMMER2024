...
# env = sim.Environment("models/robot.xml", "models/slide_scene15deg300h.xml")
...
def controller(model, data):
    ...
    X_des = np.array([prev_p, prev_alpha, beta_angle_correction, 4, 0, 0])
    u_0 = np.array([0,0])
    u = u_0 - K @ (X - X_des)
    
    print(f"des X {np.round(X_des,2)}")
    ...
    # if keyboard.is_pressed('p'):
    #     des_pos_l = np.array([0, -0.648185424949238+0.0825 + del_pos_l + 0.2])
    #     des_pos_r = np.array([0, -0.648185424949238+0.0825 + del_pos_r + 0.2])
    # elif keyboard.is_pressed('o'):
    #     des_pos_l = np.array([0, -0.648185424949238+0.0825 + del_pos_l])
    #     des_pos_r = np.array([0, -0.648185424949238+0.0825 + del_pos_r])
    # elif keyboard.is_pressed('i'):
...
ax[-1].plot(time_arr, states.T[:,0], "-",linewidth = 1.5, label=("beta"))
ax[-1].set_xlabel("Время, [c]")
ax[-1].set_ylabel("Положение, [м]")
ax[-1].grid()

plt.savefig("footages/test_49_max_spd.svg")
