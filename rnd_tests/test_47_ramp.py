...
env = sim.Environment("models/robot.xml", "models/slide_scene15deg300h_one.xml")
...
def controller(model, data):
    ...
    # prev_v -= np.sign(prev_v)*0.1
    # prev_dalpha -= np.sign(prev_dalpha)*0.1
    ...
    if keyboard.is_pressed('w'):
        ...
        alpha_des = prev_alpha
        v_des = low_pass_filter(prev_v, 0.5, 0, 1e-3)
        prev_dalpha = low_pass_filter(prev_dalpha, 0.5, 0, 1e-3)
        p_des = prev_p 
    ...
    dalpha = data_sensors.chassis_imu.gyro[2]
    ...
    if np.abs(prev_l_lqr - l_pendulum) >= 0.2:
        ...
        K = lin_model.calculate_lqr(np.diag([2,6.37,39.06,0.25, 0.19, 11.11]), np.diag([0.03, 0.03]), l_pendulum)
        # K = lin_model.calculate_lqr(np.diag([2,6.37,39.06,2, 0.19, 11.11]), np.diag([0.03, 0.03]), l_pendulum)
    ...
    k_torque_correction = 20
    
    # if np.round(gamma,2) > 0:
    #     del_pos_l = err_gamma * k_gamma
    #     # left_wheel -= 6 * np.abs(del_pos_l) if np.abs(del_pos_l) <= 0.5 else 0.5 * 5
    #     # left_wheel -= k_alpha * X[1]
    # elif np.round(gamma,2) < 0:
    #     del_pos_r = -err_gamma * k_gamma
        # right_wheel += 6 * np.abs(del_pos_r) if np.abs(del_pos_r) <= 0.5 else 0.5 * 3
        # right_wheel -= k_alpha * X[1]
        
    # if np.round(act_pos_r[1] - act_pos_l[1], 2) > 0:
    #     left_wheel += k_torque_correction * (act_pos_r[1] - act_pos_l[1])
    # elif np.round(act_pos_l[1] - act_pos_r[1], 2) > 0:
    #     right_wheel += -k_torque_correction * (act_pos_l[1] - act_pos_r[1])
    ...
    #     des_pos_l = np.array([0, -0.648185424949238+0.0825 + 0.2])
    #     des_pos_r = np.array([0, -0.648185424949238+0.0825 + 0.2])
    #     beta_angle_correction = 0.29
...
# ax[-1].plot(time_arr, states.T[:,3]*3.6, "-",linewidth = 1.5, label="p")
# ax[-1].set_xlabel("Время, [c]")
# ax[-1].set_ylabel("Скорость, [км/ч]")
# ax[-1].grid()
# ax[-1].legend()
ax.append(plt.subplot(1, 1, 1))
ax[-1].plot(time_arr, np.rad2deg(states.T[:,2]), "-",linewidth = 1.5, label=("beta"))
ax[-1].set_xlabel("Время, [c]")
ax[-1].set_ylabel("Наклон, [град]")
ax[-1].grid()

plt.savefig("footages/test_47_ramp.svg")
