...
def controller(model, data):
    ...
    x_curr = prev_x + Del_s/Del_alp * (np.sin(alpha) - np.sin(prev_alpha)) if not np.isclose(Del_alp, 0) else 0
    y_curr = prev_y - Del_s/Del_alp * (np.cos(alpha) - np.cos(prev_alpha)) if not np.isclose(Del_alp, 0) else 0
...
plt.xlabel("Время, [c]")
plt.ylabel("Высота корпуса, [м]")
plt.plot(time_arr, actual_com_robot, "-",linewidth = 1.5, label="actual")
plt.grid()
plt.savefig("footages/test_43_height.svg")
